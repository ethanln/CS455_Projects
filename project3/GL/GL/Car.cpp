...
void Car::moveCarForward(float rate)
{
	/*MOVE CAR FORWARD*/
	this->orientation += (rate * this->getWheelDirection());

	this->x_pos -= rate * sin(math_ext::toRads(this->orientation));
	this->z_pos -= rate * cos(math_ext::toRads(this->orientation));
}

void Car::moveCarBackward(float rate)
{	
	/*MOVE CAR BACKWARD*/
	this->orientation -=  (rate * this->getWheelDirection());
