...
void createObject(string _object_uri, string _texture_file);
void draw();
...
int main(int argc, char **argv)
{
	...
	matrix4 mat = graphics_pipeline::getProjectionMatrix();

	

	/*END OF PIPELINE TESTING*/

	// SET UP:
	init(argc, argv);

	// SET UP OBJECTS:
	//createObject("objects/crayonbox-color-green.obj", "objects/greenCrayon.bmp");
...
void init(int argc, char** argv)
{
	...
	cam = new camera(0.0f, 0.0f, 0.0f, -20.0f);
...
void GLKeyDowns(unsigned char key, int x, int y)
{
	...
	if (key == 'w') {
		//camZ += 0.4f;
		cam->setCamZ(cam->getCamZ() + 0.5f * sin((((cam->getOrientation() + 90.0f) * M_PIs) / 180.0f)));
		cam->setCamX(cam->getCamX() + 0.5f * cos((((cam->getOrientation() + 90.0f) * M_PIs) / 180.0f)));
...
void GLMouseMovements(int x, int y)
{
	...
	if (x < lastxs)
	{
		cam->setOrientation(cam->getOrientation() - 70.0f);
	}
	else
	{
		cam->setOrientation(cam->getOrientation() + 70.0f);
